...
using UnityEngine.SceneManagement; //We're using generic collections, specifically List<string>, so we have to add this directive
...
public class Manager3 : MonoBehaviour
{
	...
	void OnMouseDown()
	{
		...
		if (currentQuestion >= 15) //Now all questions answered, so it's time to give a result.
		{ 		if (Input.GetMouseButtonDown (0)) {
			...
			{
				QuestionTextMesh.text = "peace at last"; //Cat wants to live with you
				CatSpriteRenderer.sprite = CatHappy; //Set cat sprite
			}
			else
			{
				QuestionTextMesh.text = "you are what you eat \n "; //Cat can't live with you
